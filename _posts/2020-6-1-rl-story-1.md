...
이해를 돕기 위해 앞으로 소개할 구현체들의 전체적인 짜임새를 크게 수정하지는 않을 것이다. 여러 방법론들 간 어떤 부분들이 비슷하고 다른지 따져 가며 읽어 보기를 권장한다.

```python
"""
Note that there will be large degree of redundancy around every codes.
...
for i in range(1, 10000):
    ...
    It's because your policy can only leverage the data made by itself.
    This makes policy gradient algorithms sample-inefficient. We will fix this.
    """
    STATE_MEMORY = []
    ACTION_MEMORY = []
    REWARD_MEMORY = []
    ...
    # Without this part, It simply reduces to a supervised learning problem.
