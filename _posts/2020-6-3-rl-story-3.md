...
지난 이야기에서 구현했던 모든 방법론들에서 가치 함수를 추정하는 부분을 시간차 학습으로 교체할 수 있다. 행동 교정의 신호로 $$r + V^{\pi}(s') - V^{\pi}(s)$$를 사용할 수도 있고 또 그것이 표준에 가깝지만 $$Q^{\pi}$$, 내지는 $$Q^{\pi} - V^{\pi}$$를 사용하는 것도 얼마든지 가능하다.
...
$$Q^{\pi}$$를 시간차 학습으로 추정하기 위해 필요한 데이터는 $$(s,\ a,\ r,\ s',\ a')$$다. 정책 $$\pi$$가 개입한 흔적은 데이터 $$a'$$에서 드러난다. 지난 이야기에서 꼼꼼히 따져 봤다. 
...
optimizer_actor = torch.optim.AdamW(policy.parameters(), lr = LEARNING_RATE)
optimizer_critic = torch.optim.AdamW(q.parameters(), lr = 5*LEARNING_RATE)
