...
$$V^{\pi}$$를 시간차 학습으로 추정하는 방법과 같은 논리로 $$Q^{\pi}$$를 시간차 학습으로 추정할 수 있다. 

$$Q^{\pi}(s, a) \approx r + Q^{\pi}(s', a')$$

$$Q^{\pi}$$를 시간차 학습으로 추정하기 위해 필요한 데이터는 $$(s,\ a,\ r,\ s',\ a')$$다. 정책 $$\pi$$가 개입한 흔적은 데이터 $$a'$$에서 드러난다. 지난 이야기에서 꼼꼼히 따져 봤다. $$Q^{\pi}(s, a)$$를 행동 교정의 신호로 사용할 수도 있다.
...
$$Q^{\pi}$$를 시간차 학습으로 추정하는 논리를 유심히 관찰해두자. 이후 이야기들의 뼈대가 된다. 
