...
$$V^{\pi}$$를 시간차 학습으로 추정하는 방법을 논했다. 같은 논리로 $$Q^{\pi}$$ 역시 시간차 학습으로 추정할 수 있다. 

$$Q^{\pi}(s, a) \approx r + Q^{\pi}(s', a')$$

$$Q^{\pi}$$를 시간차 학습으로 추정하기 위해 필요한 데이터는 $$(s,\ a,\ r,\ s',\ a')$$다. 정책 $$\pi$$가 개입한 흔적은 데이터 $$a'$$에서 드러난다. 지난 이야기에서 꼼꼼히 따져 봤다. 다음 이야기로 넘어가기 전에 직접 구현해보기를 권장한다. 이번 이야기와 지난 이야기의 코드들을 적절히 섞으면 가능하다.
...
내가 기대한 자극 $$V^{\pi}(s) - V^{\pi}(s')$$과 실제로 받은 자극 $$r$$의 간극이 줄어들도록 나의 기대를 바꾸어 나간다. 전형적인 조건 형성이다. 조건 형성은 자극의 의외성에 달렸다. 받는 자극이 더 의외일수록 조건 형성은 더 강하게 일어난다. 심리학의 레스콜라-바그너 모형이 조건 형성을 설명하는 논리다. 시간차 학습과 놀랍도록 닮아 있다. 삶의 경험이 늘어나면 이 오차는 작아진다. 모든 자극을 예상할 수 있다. 삶이 지루해진다.
